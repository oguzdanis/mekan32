...
const cevapOlustur = function(res,status,content){
  ...
  .json(content)
	...
	if(gelenMekan.yorumlar && gelenMekan.yorum.length > 0) {
...
const yorumGuncelle = function(req, res) {
    ...
    //- işaretri yorumlar ve puan dışında her şeyi almamızı sağlar.
    .select('yorumlar')
    .exec(
      function(hata,gelenMekan) {
      	var yorum;
...
const yorumSil = function(req, res) {
	if(!req.params.mekanid || !req.params.yorumid) {
		cevapOlustur(res,404, {"mesaj": "Bulunamadı. mekan ve yorumid gerekli."});
		return;
	}
    Mekan
    .findById(req.params.mekanid)
